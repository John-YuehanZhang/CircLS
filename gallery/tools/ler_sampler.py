...
into graphlike components; otherwise the hypergraph decoder mwpf, with
the same edge folding and per-shot solve timeout as the paper's
experiments (experiments/formula_deviation.py).  The decoder is built
once in the parent, surfacing any failure immediately; fork()ed workers
inherit it.
"""
import math
