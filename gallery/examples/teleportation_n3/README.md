...
One program of the paper's Table 2: `teleportation_n3` (QASMBench), compiled by CircLS in the paper's configuration, plus the ancilla-orientation rule added on 2026-09-25
(each |+> gadget ancilla is born facing the step that consumes it, `magic_proxy="X"`), which leaves this
program unchanged (both ancillas already face their partner), so the numbers equal the paper's Table 2 cells.
