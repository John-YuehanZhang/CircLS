...
class QECSystem:
    ...
    # ======================================================================
    @staticmethod
    def _stabilizer_signature(global_stab: Dict[str, Any]):
        """Identity of a PHYSICAL check: ``(type, support, ancilla)``.

        Registration deduplicates on this key so that the SAME physical check
        keeps its uid when it changes owner (grow_patch/move_corners/shrink_patch
        re-registration, a patch reborn on cells an absorbed corridor borrowed)
        — that is what continues its detector history.

        The ancilla (``syn_idx``, the global index of the syndrome qubit) is part
        of the identity: a check is measured BY an ancilla, so the same operator
        read out on a different qubit is a different physical check.  Leaving it
        out aliased two distinct checks onto one uid — e.g. a corridor's weight-2
        seam check and a patch's identical-support boundary lobe sitting on
        different ancillas — and the two registration paths then corrupted each
        other's records: ``add_patch`` overwrote the live record (moving an ACTIVE
        patch check onto the corridor ancilla, and hiding it from
        ``activate_coupler``'s syn_coord-keyed pause list), while
        ``_apply_patch_geometry`` kept the STALE record (handing the deformed
        patch a check on a foreign ancilla).  Either way one ancilla ended up
        carrying two active checks, which the compact-7 schedule validator
        reports as "slot N on qubit (x, y) is used by both X-check@... and
        Z-check@..." (bv_n70, 2026-08-14).

        ``syn_idx`` is None for checks without an ancilla (e.g. PQRM X stabs);
        those keep the old (type, support) behaviour.  Indices — not coords —
        are the stable identity: they survive ``rotate_patch`` (which permutes
        coords inside a patch) and are never handed out twice.
        """
        return (global_stab['type'],
                tuple(sorted(global_stab['data_indices'])),
                global_stab.get('syn_idx'))
    ...
    def add_patch(self, patch: QECPatch, offset: Tuple[float, float] = (0, 0), name: str = None, is_active: bool = True):
        ...
        # Atomicity pre-pass: check EVERY coordinate for active collisions
        # BEFORE mutating any registry — a blocked add must leave no partial
        # state (the blocked-repair fallback re-registers the same name, and
        # a half-registered patch turns the retry into a bogus
        # "already exists" error).
        for global_coord in patch.index_map:
        ...
        for global_coord, local_index in patch.index_map.items(): # the patch is already shifted to the global coordinate system
            ...
            if global_coord in self.index_map:
                ...
                # A reborn cell is FRESH: clear its retired flag (same
                # revive as _apply_patch_geometry) — the birth init that
                # follows resets its records, and a later deformation
                # (move_corners) must see the full live footprint.
                if self._tracker is not None:
                    self._tracker.retired_qubits.discard(existing_idx)
                idx = existing_idx
            else:
        ...
        for stab in patch.stabilizers:
            ...
            signature = self._stabilizer_signature(global_stab)

            if signature in self._stabilizer_signatures:
                existing_uid = self._stabilizer_signatures[signature]
                # the signature can match a DEAD region's record (e.g. a patch
                # born on cells an absorbed corridor borrowed, first_use_init).
                # Take ownership with the freshly-translated record — same fix
                # as grow_patch below; keeping the stale record leaves
                # patch_name (and any corridor metadata) pointing at the dead
                # owner, so every patch_name-keyed scan (litinski bulk probe,
                # retire, SE selection) misses the revived check.  Because the
                # signature pins the ANCILLA too, the overwritten record
                # describes the very same physical measurement (same type, same
                # support, same syndrome qubit) — the uid, and with it the
                # detector history, carries over legitimately.  Without the
                # ancilla in the key this branch used to MOVE a live check onto
                # a foreign ancilla: a coupler patch references patch data
                # qubits it does not own (so step 2's collision guard never
                # fires for them), and its identical-support seam check would
                # overwrite the patch's still-ACTIVE boundary lobe.
                self.stabilizers[existing_uid] = global_stab
    ...
    def grow_patch(self, name: str, new_patch: QECPatch, offset: Tuple[float, float] = (0, 0)):
        ...
        signature is ``(type, global data indices, ancilla index)`` — every check that the
        enlarged code shares with the old one (same support ON THE SAME ancilla)
        **keeps its uid** and therefore its detector history.  That
        ...
        and the handoff window is fully detector-spanned by those
        bridging detectors.
    ...
    def _apply_patch_geometry(self, name: str, grown: QECPatch,
        ...
        for stab in grown.stabilizers:
            ...
            g['patch_name'] = name
            signature = self._stabilizer_signature(g)
            ...
            if uid is None:
                ...
                # Install the freshly-translated record (same fix as add_patch):
                # the signature pins type + support + ANCILLA, so this is the
                # same physical check and the uid (its detector history) carries
                # over, but every other field — syn_coord, per-qubit paulis,
                # corridor metadata — must be the CURRENT geometry's, not the
                # dead owner's.  Keeping the stale record used to hand the
                # deformed patch a check sitting on a foreign ancilla.
                self.stabilizers[uid] = g
    ...
    def shrink_patch(self, name: str, new_patch: QECPatch,
        ...
        # dormant-reuse ledger sync for THIS patch's syndrome qubits:
        # active <=> referenced by an active stabilizer row.  Protocol stage
        # swaps (litinski) REPLACE boundary w2 rows; without the sync the
        # replaced rows' syn qubits stay active forever — a later corridor
        # registration collides on them (twistedghz_4 at (26,14),
        # 2026-08-05) — while the replacement rows' syns read dormant, so a
        # later add_patch could hand their index out while the row still
        # measures them.
        # DEACTIVATE-ONLY on purpose: qubits enter the ledger via the
        # builder's initialize, never via registration (add_patch/grow_patch
        # deliberately do not populate it) — an activate direction here
        # re-armed syn qubits the liveness-reuse flow had freed and
        # collided teleport_4-liveness at (26,26) (measured 2026-08-05).
        mine = {q for q in self.syndrome_indices
                if self.index_to_owner_map.get(q) == name}
        if mine:
            referenced = set()
            for uid in self.active_stabilizer_indices:
                si = self.stabilizers[uid].get('syn_idx')
                if si is not None:
                    referenced.add(si)
            self.active_qubit_indices -= (mine - referenced)
    ...
    def activate_coupler(self, coupler_name: str):
        ...
        if coupler_name in self.paused_stabilizer_indices:
            import warnings
            warnings.warn(f"Coupler '{coupler_name}' is already active.")
            return
    ...
    def deactivate_coupler(self, coupler_name: str):
        ...
        if coupler_name not in self.paused_stabilizer_indices:
            import warnings
            warnings.warn(f"Coupler '{coupler_name}' is not currently active "
                          "(or wasn't activated via this method).")
            return
    ...
    def retire_measured_patch(self, name):
        ...
        if self._tracker is not None:
            ...
            self.num_logicals = self._tracker.expected_num_logicals
        # Make the retired qubits DORMANT so their coarse cells can be reused as a
        ...
        # SYNDROME qubits retire too — their stabilizers are dead above, and a corridor
        # with X legs lands on syndrome cells of the freed patch (an all-Z corridor only
        # ever borrowed data cells, which is why data-only retirement used to suffice).
        # They stay out of the tracker's retired set: only data qubits carry records.
        sq = {q for q in self.syndrome_indices
              if self.index_to_owner_map.get(q) == name}
        self.active_qubit_indices.difference_update(dq | sq)
