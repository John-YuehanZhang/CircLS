...
# Canonical definition lives in tableau.py (the record-merge honours it); re-exported here for existing importers.
from .tableau import UNMEASURED_STAB_RECORD
...
class SyndromeTracker:
    ...
    def record_absorbed_op(self, op: np.ndarray) -> bool:
        ...
        (Z1).  Returns True iff a new row was banked.

        Unlike upstream's operator-only ledger, every banked row appends an
        EMPTY records row: retire_qubits and the resolve-at-readout path
        filter absorbed_ops.records positionally alongside the matrix, so
        the two must stay the same length.

        CircLS deviation: the mid-measurement PPM absorb sites (single,
        combination, half-read re-pricing) do NOT come through here — at
        their insertion moment the joint's closure row already sits in the
        stabilizer bank, and reducing against it would cancel the banked
        DOF against its own reflection (measured: the liveness census
        tripped).  They keep their bank-only rank-growth dedup."""
    ...
    def validate_logical_count(self, *, context: str = "tracker state") -> None:
        ...
        actual = self.logicals.count + self.num_absorbed_dof()
        if actual != self.expected_num_logicals:
            raise RuntimeError(
                f"After {context}: logical count {self.logicals.count} plus "
                f"absorbed logical DOFs {self.num_absorbed_dof()} != "
    ...
    def process_mid_measurement(self,
                            ...
                            for c_idx in comp_indices:
                                ...
                                for r in full_records[c_idx]:
                                    ...
                                    if rec_to_append in args_set:
                                        ...
                                        args_set.add(rec_to_append)  # addition modulo 2
                            args = sorted(args_set,
                                          key=lambda target: target.value)

                            # any sentinel-tagged component row means the
                            # combination's init parity involves an UNWATCHED
                            # gauge direction — the `r < 0: continue` above
                            # dropped the tag, but the closure is still
                            # per-shot random: suppress the detector (same
                            # rule as the terminal-readout consumer)
                            _gauge_tainted = any(
                                UNMEASURED_STAB_RECORD in full_records[c_idx]
                                for c_idx in comp_indices)
                            if (not _gauge_tainted
            ...
            for ri in new_basis_indices:
                ...
                if orig_idx >= num_stabs:
                    ...
                    if (_n_surviving + _n_promoted + _absorbed_pre + _pending
                        ...
                        # by every record consumer.  Write the sentinel into
                        # full_records (the ORIGINAL, keyed by orig_idx): every
                        # record consumer below reads full_records[i] — the
                        # logical rebuild (self.logicals.records) and
                        # old_stab_records — NOT the reordered copy.  When
                        # num_logs>0 reordered_records is a fresh list, so
                        # rebinding reordered_records[ri] was a dead store that
                        # left the gauge row records-less, and its close-out
                        # parity leaked out as a forbidden detector (fix
                        # 2026-08-24).
                        full_records[orig_idx] = [-1]
                        if _os.environ.get('LIGHTSTIM_DEBUG_ABSORB'):
                ...
                for _gi in _gauge_filed:
                    ...
                    for _k, _c in enumerate(_log_pivots):
                        if full_matrix[_gi, _c]:
                            ...
                            _purged = True
                    if _purged and _os.environ.get('LIGHTSTIM_DEBUG_RESOLVE'):
                        print(f"[resolve-debug] gauge row @{_gi} purged of "
                              f"standing-logical content")
        ...
        if len(_round_cands) >= 2 and num_logs > 0 and len(_round_cands) <= 8:
            ...
            # sentinel-tagged rows ([-1]) are UNWATCHED gauge directions:
            # record-less in substance despite the truthy tag, so they must
            # not certify a combination as code-determined (same exclusion
            # as the readout consumer's `-1 in full_records[k]` skip).
            _rec_rows = [k for k in range(num_stabs)
                         if full_records[k]
                         and UNMEASURED_STAB_RECORD not in full_records[k]]
        ...
        num_absorbed = self.num_absorbed_dof()
    ...
    def process_data_measurement(self,
        ...
        if A.count and num_new_meas:
            ...
            if resolve_absorbed:
                ...
                for r in range(A.count):
                    ...
                    if _os.environ.get('LIGHTSTIM_DEBUG_RESOLVE'):
                        print(f"[resolve-debug] absorbed row {r} touched by "
                              f"readout, remainder="
                              f"{'nonzero' if (_depf[0] and _rem.any()) else 'none'}")
            else:
            ...
            for _rem in _half_read_rems:
                ...
                import os as _os
                if _os.environ.get('LIGHTSTIM_DEBUG_RESOLVE'):
                    print(f"[resolve-debug] re-priced remainder: folded "
                          f"standing row {_kill} (of {_lcomp}) into "
                          f"absorbed_ops")
        ...
        for k in range(num_rows):
            ...
            if k < num_stabs and k not in self.stabilizer_with_logical_components:
                ...
                # are emitted like every other row: the long-range parity is
                # real syndrome information — paired-noise MWPM LER is ~30%
                # better with it (2026-08-09 probes; PR#68 review blocker #1
                # reversed the 2026-07-30 weight-heuristic ruling).
                if -1 in full_records[k]:
                    ...
                    # Y(x)Y mechanism).  Same design decision as above.
                    continue
