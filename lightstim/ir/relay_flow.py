...
def solve_relay_round(chunk: stim.Circuit,
    ...
    # --- 5c. de-nest the round's detector basis ---------------------------
    # The Zassenhaus completion rows of the intersection basis are heavy
    # GF(2) combinations; emitted raw, their record sets nest (D_a
    # strictly inside D_b inside D_c), so a single fault on a shared
    # record lights 5-7 detectors at once and the minimal pairs that
    # stim's decomposition needs as bridges exist only as XORs of
    # emitted rows -- from d = 5 up the round's error model stops being
    # graphlike and forces a hypergraph decoder.  Row-reduce to the
    # minimum-weight coset representatives instead: whenever one
    # detector's record set strictly contains another's, XOR the
    # contained one out.  Same count, same GF(2) span (hence identical
    # error-model probabilities and identical undetectable-fault set);
    # measured on the eight t_as_s d = 5 programs this restores native
    # decomposability.
    changed = True
    while changed:
        changed = False
        for i in sorted(range(len(detectors)),
                        key=lambda k: -len(detectors[k])):
            if len(detectors[i]) <= 2:
                continue
            best = None
            for j in range(len(detectors)):
                if (j != i and 0 < len(detectors[j]) < len(detectors[i])
                        and detectors[j] <= detectors[i]):
                    if best is None or len(detectors[j]) > len(detectors[best]):
                        best = j
            if best is not None:
                detectors[i] = detectors[i] ^ detectors[best]
                changed = True
