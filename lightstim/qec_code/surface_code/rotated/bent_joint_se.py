...
def assign_hook_benign_schedule(checks, domains=None, default=_DEFAULT_ORIENTATION,
    ...
    # iterative backtracking (was recursive; depth = len(order) reaches
    # thousands of checks on big boards — ghz_n127 blew the Python recursion
    # limit, 2026-08-04).  iters[i] persists each depth's candidate cursor
    # across backtracks, exactly like the recursive frame did.
    i = 0
    iters = {0: iter(idle_options)}
    nodes[0] += 1
    while i < len(order):
        ...
        for idle in iters[i]:
            ...
            if consistent(k, tm):
                ...
                for q, t in tm.items():
                    occupied[(t, q)] = k
                i += 1
                iters[i] = iter(idle_options)
                nodes[0] += 1
                break
        else:
            # candidates exhausted at this depth: undo the parent and resume
