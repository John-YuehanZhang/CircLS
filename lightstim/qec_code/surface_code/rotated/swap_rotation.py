...
def _prep_rotation(system, name):
    ...
    RETIRED qubits are not part of the live patch: a previous litinski
    rotation measures out the rows it grew through and leaves them
    owned-but-retired, so counting them reads a d x d patch as a
    d x (2d-1) rectangle (``|data|=15`` at d=3) at a shifted origin.  Same
    filter as ``litinski_rotation._litinski_stage_gen``."""
