...
RUNNER = REPO / "lightstim" / "benchmarks" / "memory" / "run_memory.py"
PYTHON = Path(sys.executable)              # the running interpreter, not a hardcoded venv

sys.path.insert(0, str(REPO))
sys.path.insert(0, str(REPO / "lightstim" / "benchmarks" / "memory"))
...
def _has_bposd() -> bool:
    ...
    """
    try:
        from lightstim.simulation.decoder_backend.registry import has_decoder
        return has_decoder("bposd", "cpu")
    except Exception:
        return False
...
@pytest.mark.slow
@pytest.mark.skipif(
    not (_has_cuda() and os.environ.get("RUN_GPU_TESTS")),
    reason="needs CUDA and RUN_GPU_TESTS=1 (shared-machine opt-in)")
