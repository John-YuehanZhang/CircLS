...
# Gidney's zenodo 7487893 reference implementation (code/src);
# set GIDNEY_SRC to run the cross-checks against it.
GIDNEY_SRC = os.environ.get("GIDNEY_SRC", "")
...
def _import_gidney():
    if not hasattr(np, 'bool8'):        # his package predates the numpy 2 removal
        np.bool8 = np.bool_
    if not GIDNEY_SRC or not os.path.isdir(GIDNEY_SRC):
        pytest.skip("set GIDNEY_SRC to the zenodo 7487893 code/src dir")
