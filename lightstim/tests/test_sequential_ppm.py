...
distance d at p=1e-3):
  ...
  * the cap-slot regression (transposed row-3 E/W wall);
...
def test_row3_transposed_ew_wall_cap_slot():
    # cap-slot regression: X⊗Z through the E/W (vertical) seam —
...
def test_independent_of_circls():
    # in-process guard (cheap) — only meaningful when this suite runs
    # alone; a combined run imports circls first, so skip the weak form
    # and rely on the fresh-interpreter check below
    if 'circls' in sys.modules:
        pass
    else:
