...
VOCABULARY (the user's, non-negotiable): **textbook/conjugate = the weight-2
...
registered at offset (0, 0).  Multi-patch rule (Phase 2 hosts the geometry;
the selector is live now): the type class with FEWER patches carries the
stretched stabilizers.
"""
...
from .multi_patch_coupler import BentLayoutError
...
def detect_layout(system, name, d, origin):
    ...
    checkerboard phase, read off its records (promoted from
    ``swap_rotation._detect_layout``)."""
    ox, oy = origin
    lr = tb = None
    bulk_type = None
...
def patch_view(system, name):
    ...
    RETIRED qubits are NOT part of the live footprint: a deformation
    protocol (litinski rotation) measures out the rows it grew through and
    leaves them owned-but-retired, so counting them reads the patch as a
    d x (2d-1) rectangle ("not square") at a shifted origin.  Same filter as
    ``litinski_rotation._litinski_stage_gen``."""
    owner = system.index_to_owner_map
    retired = (system._tracker.retired_qubits
               if getattr(system, '_tracker', None) is not None
               else frozenset())
...
def assign_walls(views, target, seams):
    ...
    patches by TYPE and pick the class with FEWER patches (the user's rule;
    e.g. 1 textbook + 3 conjugate -> the textbook patch's seams get the
    walls).  Tie (2-2) -> the lexicographically smaller name set.  Returns
    {seam: bool} — a seam carries a wall iff it touches a selected patch.
    Phase 1 uses this for the single seam; Phase 2 hosts multi-seam walls."""
...
def _wall_checks(d, phi_near, phi_far, x0, y0, axis, sgn_override=None):
    ...
    # cap end: the alternating-spacing rule decides —
    ...
    # diverges on transposed row-3 (M8: cap crammed next to existing corner
...
def wall_spec(a, b, target):
    ...
    # alternating-spacing rule ("add one, skip one gap"): the
    ...
    if not free:
        raise SeamRuleError(
            ...
            f"(alternating-spacing rule): both end rows "
